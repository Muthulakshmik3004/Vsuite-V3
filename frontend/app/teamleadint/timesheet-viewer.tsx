...
const TeamLeadTimesheetViewer = () => {
  ...
  // State for review modal (kept for compatibility)
  ...
  }, [reviewNotes, fetchAllEmployeesTimesheets]);

  const handleRejectTimesheet = useCallback(async (employeeId: string, date: string) => {
    const rejectionNote = `${rejectionReason}: ${reviewNotes}`.trim();
    try {
      const result = await timesheetService.reviewTimesheet({
        employee_id: employeeId,
        date: date,
  ...
  }, [reviewNotes, rejectionReason, fetchAllEmployeesTimesheets]);
  ...
  const handleBulkExportExcel = useCallback(async () => {
    try {
      ...
      if (result.success) {
        Alert.alert('Success', 'Bulk Excel export downloaded successfully');
      } else {
        Alert.alert('Error', result.message);
      }
    } catch (error) {
          ...
          <View style={styles.expandedContent}>
            ...
            <View style={styles.approvalButtons}>
              <TouchableOpacity
                ...
                onPress={() => {
                  setReviewingTimesheet(`${item.employee_id}|||${item.date}`);
                  setRejectionReason('');
                  setReviewNotes('');
            ...
            {item.activities && item.activities.length > 0 && (
              <View style={styles.activitiesSection}>
                ...
                {item.activities.map((activity: any, index: number) => (
                  <View key={`${item.employee_id}-${item.date}-${activity.time}-${index}`} style={styles.activityItem}>
        ...
        {allEmployeesData?.employees && allEmployeesData.employees.length > 0 ? (
          <View style={styles.listContainer}>
            <FlatList
              ...
              keyExtractor={(item) => `${item.employee_id}-${item.date}`}
              scrollEnabled={false}
            />
          </View>
        ) : loading ? (
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBody}>
              <Text style={styles.modalText}>
                Please review the timesheet and select an action.
              </Text>
              ...
              <View style={styles.inputGroup}>
                ...
                <View style={styles.pickerWrapper}>
                  <Picker
                    ...
                    onValueChange={(itemValue) => setRejectionReason(itemValue)}
                    style={styles.picker}
                  >
                    <Picker.Item label="Select reason..." value="" />
                    <Picker.Item label="Incomplete activities" value="Incomplete activities" />
                    <Picker.Item label="Inaccurate time entries" value="Inaccurate time entries" />
                    <Picker.Item label="Missing project details" value="Missing project details" />
                    <Picker.Item label="Invalid activity types" value="Invalid activity types" />
                    <Picker.Item label="Time overlap issues" value="Time overlap issues" />
